...
const CommunityFeed = ({ isDarkMode }) => {
  // Post data state
  ...
  return (
    <div className="max-w-3xl mx-auto px-2 sm:px-6 lg:px-8">
      ...
      {/* New Post Form */}
      {showNewPostForm && (
        <div className={`mb-6 p-4 rounded-lg ${isDarkMode ? "bg-zinc-800" : "bg-white shadow-md"}`}>
          <div className="mb-4">
            <textarea
              value={newPostContent}
              onChange={(e) => setNewPostContent(e.target.value)}
              placeholder="What's on your mind?"
              className={`w-full min-h-[100px] p-3 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2 sm:justify-between">
            <div className="flex gap-2 w-full sm:w-auto justify-center sm:justify-start">
              <button className={`p-2 rounded-full ${isDarkMode ? "hover:bg-zinc-700 text-zinc-400" : "hover:bg-zinc-100 text-zinc-500"}`}>
            ...
            <div className="flex gap-2 w-full sm:w-auto justify-center sm:justify-start mt-2 sm:mt-0">
              <button
                onClick={() => setShowNewPostForm(false)}
                className={`px-4 py-2 rounded-lg font-medium ${
                  isDarkMode ? "text-zinc-300 hover:bg-zinc-700" : "text-zinc-600 hover:bg-zinc-100"
                }`}
              >
                Cancel
              </button>
